...
import { generateInstagramContent, updateSlide, testGeminiConnection, type CarrosselOut } from "@/lib/gemini.functions";
...
export const Route = createFileRoute("/carrossel")({
  head: () => ({ meta: [
    ...
    { name: "description", content: "Gere carrosséis de Instagram com IA (Gemini) a partir de tema, objetivo, público e tom." },
    { property: "og:title", content: "Gerar Carrossel — InLabs.Ia Studios" },
    { property: "og:description", content: "Carrosséis personalizados com Google Gemini." },
  ]}),
...
function NovoCarrossel() {
      ...
      await Promise.all(out.slides.map(async (s) => {
        try {
          ...
          const r = await genImgTop({ data: {
            ...
            slideBody: s.texto,
            slideIndex: s.numero,
            slideTotal: out.slides.length,
            slideKind: s.tipo,
            style: `${form.tom}, tema: ${form.tema}`,
          }});
          setAutoImgs(prev => ({ ...prev, [s.numero]: r.dataUrl }));
        } catch (err) {
  ...
  async function runTest() {
    ...
    try {
      ...
      if (r.ok) toast.success("Gemini conectado"); else toast.error(r.message);
    } catch (e) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-3xl mx-auto p-6 lg:p-10">
        <div className="inline-flex items-center gap-2 text-xs uppercase tracking-widest text-muted-foreground mb-2">
          <Sparkles className="w-3 h-3"/> Novo carrossel Gemini
        </div>
        ...
        <form onSubmit={submit} className="bg-card border border-border rounded-2xl p-6 space-y-4">
          ...
          <div className="space-y-2">
            <button type="button" onClick={runTest} disabled={testing}
              ...
              {testing ? <><Loader2 className="w-4 h-4 animate-spin"/> Testando conexão Gemini...</> : <><Plug className="w-4 h-4"/> Testar conexão Gemini</>}
            </button>
            {testResult && (
          ...
          <button type="submit" disabled={busy}
            ...
            {busy ? <><Loader2 className="w-4 h-4 animate-spin"/> Gerando com Gemini...</> : <><Wand2 className="w-4 h-4"/> Gerar carrossel</>}
          </button>
          <p className="text-[11px] text-muted-foreground text-center">Movido a Google Gemini • Limite de 30 gerações por dia por chave.</p>
        </form>
