...
// ---------------------------------------------------------------------------
// TEXTO — Google Gemini, exclusivamente via API REST oficial (generateContent)
// Nunca usar @google/genai (SDK) nem interactions.create() (API legada/removida).
// ---------------------------------------------------------------------------
...
async function callChat(messages: { role: string; content: string }[]): Promise<string> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`A Gemini retornou um erro (${response.status}). Tente novamente.`);
    }
    ...
    if (err.name === "AbortError") throw new Error("Tempo esgotado ao chamar a Gemini. Tente novamente.");
    if (err.message?.startsWith("A Gemini") || err.message?.includes("Gemini")) throw err;
    throw new Error(`Falha ao chamar a Gemini: ${err.message}`);
  } finally {
...
export const generateImage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ dataUrl: string }> => {
    ...
    const fullPrompt = hasText
      ? `Create ONE single cinematic poster-style image — NOT a mockup, NOT a slide inside a frame, NOT a Canva/mLabs presentation preview, NOT a photo with a caption card next to it. The final result must look like a real finished advertising poster / editorial photograph where the typography is composited DIRECTLY over the photograph, edge-to-edge, with no white borders, no rounded card, no "slide X of Y" wrapper, no device frame.
...
Unique variation seed: ${data.seed}.`
      : `${data.prompt}. Cinematic full-bleed poster image, edge to edge, no frame, no slide mockup. Unique variation seed: ${data.seed}.`;
