...
export const generateCartaz = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<SlideOut> => {
    const sys = `Você cria cartazes de eventos para Instagram, no nível de agências premiadas. Retorne JSON: { "title": "...", "body": "...", "imagePrompt": "..." }. Body inclui data, hora e local formatados.
imagePrompt sempre em inglês, still cinematográfico de campanha publicitária (nunca foto de banco de imagens genérica), usando uma metáfora visual conceitual ligada ao tipo de evento — ex.: objeto 3D flutuando, estátua clássica em contexto moderno, silhueta em contraluz, multidão vista de cima, elementos do evento em still-life dramático. Composição/iluminação/ângulo únicos baseados na seed.`;
