...
export const testGeminiConnection = createServerFn({ method: "POST" })
  .handler(async (): Promise<{ ok: boolean; model: string; message: string }> => {
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey) return { ok: false, model: "gemini-3.6-flash", message: "GEMINI_API_KEY não configurada no servidor." };
    const controller = new AbortController();
    ...
    try {
      ...
      if (!r.ok) {
        ...
        if (r.status === 401 || r.status === 403) return { ok: false, model: "gemini-3.6-flash", message: "Chave da Gemini inválida ou sem permissão." };
        if (r.status === 429) return { ok: false, model: "gemini-3.6-flash", message: "Limite da Gemini atingido no momento." };
        return { ok: false, model: "gemini-3.6-flash", message: `Erro Gemini ${r.status}: ${body.slice(0, 160)}` };
      }
      const j = await r.json() as { candidates?: { content?: { parts?: { text?: string }[] } }[] };
      const txt = j.candidates?.[0]?.content?.parts?.[0]?.text?.trim() ?? "";
      return { ok: true, model: "gemini-3.6-flash", message: `Conexão OK. Resposta: "${txt || "(vazia)"}"` };
    } catch (e) {
      const err = e as Error;
      if (err.name === "AbortError") return { ok: false, model: "gemini-3.6-flash", message: "Tempo esgotado ao contatar a Gemini." };
      return { ok: false, model: "gemini-3.6-flash", message: "Falha de rede ao contatar a Gemini." };
    } finally { clearTimeout(t); }
  });
